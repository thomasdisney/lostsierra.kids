...
export function MobileNav() {
  ...
  if (isNewUser) {
    ...
    // new_account: Home, Children, Family, Updates — no register if already registered
    links = [
      fullLinks[0], // Home
      ...(hasRegistration ? [] : [newUserLinks[1]]), // Register (only if no registration)
      fullLinks[2], // Updates (announcements)
      fullLinks[3], // Children
      fullLinks[4], // Family
    ];
  } else {
    links = [...fullLinks.filter(l => !(l as { parentOnly?: boolean }).parentOnly || role === "parent" || isAdmin),
      // re-add billing for parent
      ...(role === "parent" ? [fullLinks[1]] : []),
    ];
    // Actually just use fullLinks directly for parent, filtering parentOnly
    links = fullLinks.filter(l => !(l as { parentOnly?: boolean }).parentOnly || role === "parent" || isAdmin);
  }
